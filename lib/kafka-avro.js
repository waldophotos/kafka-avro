...
KafkaAvro.prototype._fetchAllSchemaTopics = Promise.method(function () {
  ...
  return Promise.resolve(axios.get(fetchAllTopicsUrl))
    .bind(this)
    .then((response) => {
      console.log('KafkaAvro._fetchAllSchemaTopics() :: Fetched total schemas:',
...
KafkaAvro.prototype._fetchSchema = Promise.method(function (schemaTopic) {
  ...
  console.log('KafkaAvro._fetchSchema() :: Fetching schema url:',
    fetchSchemaUrl);

  return Promise.resolve(axios.get(fetchSchemaUrl))
    .then((response) => {
      console.log('KafkaAvro._fetchSchema() :: Fetched schema url:',
        fetchSchemaUrl);
...
KafkaAvro.prototype._registerSchema = Promise.method(function (schemaObj) {
  console.log('KafkaAvro._registerSchema() :: Registering schema:',
    schemaObj.topic);

  try {
    schemaObj.type = avro.parse(schemaObj.schemaRaw.schema, {wrapUnions: true});
  } catch(ex) {
    console.error('KafkaAvro._registerSchema() :: Error parsing schema:',
      schemaObj.schemaTopicRaw, 'Error:', ex.message);
    return schemaObj;
  }

  console.log('KafkaAvro._registerSchema() :: Registered schema:',
    schemaObj.topic);
...
KafkaAvro.prototype._handleAxiosError = function (err) {
  ...
  console.error('KafkaAvro._handleAxiosError() :: http error:', err.message,
    'Url:', err.config.url);
