...
KafkaAvro.prototype._fetchAllSchemaTopics = Promise.method(function () {
  ...
  return Promise.resolve(axios.get(fetchAllTopicsUrl))
    .bind(this)
    .then((response) => {
      console.log('KafkaAvro :: Fetched total schemas:', response.data.length);
...
KafkaAvro.prototype._registerSchema = Promise.method(function (schemaObj) {
  try {
    ...
    console.error('KafkaAvro :: Error parsing schema:', schemaObj.schemaTopicRaw, 'Error:',
      ex.message);
    throw ex;
  }
...
KafkaAvro.prototype._handleAxiosError = function (err) {
  ...
  console.error('KafkaAvro :: http error:', err.message, 'Url:', err.config.url);
