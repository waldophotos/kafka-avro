...
Consumer.prototype.getConsumer = Promise.method(function (opts) {
  ...
  console.log('KafkaAvro.Consumer.getConsumer() :: Starting Consumer with',
    'opts:', opts);
  ...
  return new Promise(function(resolve) {
    consumer.on('ready', function() {
      console.log('KafkaAvro.Consumer.getConsumer() :: Got "ready" event.');
      resolve(consumer);
    });

    consumer.connect(function() {
      console.log('KafkaAvro.Consumer.getConsumer() :: Got "connect()" callback.');
...
Consumer.prototype._getReadStreamWrapper = function (consumerInstance, topic,
  ...
  stream.on('error', function(err) {
    console.error('KafkaAvro.Consumer._getReadStreamWrapper() :: Read Stream',
      'Error:', err);
  });
...
Consumer.prototype._onWrapper = function (consumerInstance, eventName, cb) {
  ...
  consumerInstance.__kafkaAvro_on('data', function(message) {
    if (!this.valueSchemas[message.topic]) {
      console.warn('KafkaAvro.Consumer._onWrapper() :: Warning, consumer did not find topic on SR:',
        message.topic);
    ...
    message.parsed = this.deserialize(type, message.value);
 ...
 * @param {Buffer} value The serialized message.
 * @return {*} The deserialized value.
 */
Consumer.prototype.deserialize = function (type, value) {
  var parsed = magicByte.fromMessageBuffer(type, value).value;
...
Consumer.prototype._transformAvro = function (topicName, data, encoding, callback) {
  if (!this.valueSchemas[topicName]) {
    console.warn('KafkaAvro.Consumer._transformAvro() :: Warning, consumer did not find topic on SR:',
      topicName);
  ...
  data.parsed = this.deserialize(type, data.value);
