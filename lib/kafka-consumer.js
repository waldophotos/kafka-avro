...
Consumer.prototype.getConsumer = Promise.method(function (opts) {
  ...
  return new Promise(function(resolve, reject) {
    consumer.__kafkaAvro_on('ready', function() {
      log.debug('getConsumer() :: Got "ready" event.');
      resolve(consumer);
    });

    consumer.connect({}, function(err) {
      if (err) {
        log.error('getConsumer() :: Connect failed:', err);
        reject(err);
        return;
      }
      log.debug('getConsumer() :: Got "connect()" callback.');
