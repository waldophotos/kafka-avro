  ...
  prepareSubjectName(topicName, data) {
    switch (this.strategy) {
      case SUBJECT_STRATEGIES.TOPIC_RECORD_NAME_STRATEGY:
        return (data.constructor == null || data.constructor.name === 'Object')  ? `${topicName}` : `${topicName}-${data.constructor.name}`; // If the object is not an instance of a TypeScript or Javascript class we dont have anyway to implement the `TOPIC_RECORD_NAME_STRATEGY`.
