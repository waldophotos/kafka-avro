  ...
  prepareSubjectName(topicName, data) {
    switch (this.strategy) {
      case SUBJECT_STRATEGIES.TOPIC_RECORD_NAME_STRATEGY:
        return this.isNotAClassInstance(data)  ? `${topicName}` : `${topicName}-${data.constructor.name}`; // If the object is not an instance of a TypeScript or Javascript class we dont have anyway to implement the `TOPIC_RECORD_NAME_STRATEGY`.
      case SUBJECT_STRATEGIES.TOPIC_NAME_STRATEGY:
        return `${topicName}`;
      case SUBJECT_STRATEGIES.RECORD_NAME_STRATEGY:
        return this.isNotAClassInstance(data) ? `${topicName}` : `${data.constructor.name}`; // If the object is not an instance of a TypeScript or Javascript class we dont have anyway to implement the `RECORD_NAME_STRATEGY`.
      default: // If not configured use the default `TOPIC_NAME_STRATEGY`
        return `${topicName}`;
    }
  }

  isNotAClassInstance(data) {
    return data.constructor == null || data.constructor.name === 'Object';
  }
