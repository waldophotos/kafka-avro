  ...
  prepareSubjectName(topicName, data) {
    switch (this.strategy) {
      case SUBJECT_STRATEGIES.TOPIC_RECORD_NAME_STRATEGY:
        return this.isNotAClassInstance(data)  ? `${topicName}` : `${topicName}-${data.__schemaName}`;
      case SUBJECT_STRATEGIES.TOPIC_NAME_STRATEGY:
        return `${topicName}`;
      case SUBJECT_STRATEGIES.RECORD_NAME_STRATEGY:
        return this.isNotAClassInstance(data) ? `${topicName}` : `${data.__schemaName}`;
      default: //  default `TOPIC_NAME_STRATEGY`
        return `${topicName}`;
    }
  }
